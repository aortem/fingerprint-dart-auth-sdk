...
class RelatedVisitorsScreen extends StatefulWidget {
  const RelatedVisitorsScreen({super.key});

  @override
  State<RelatedVisitorsScreen> createState() => _RelatedVisitorsScreenState();
}

class _RelatedVisitorsScreenState extends State<RelatedVisitorsScreen> {
  ...
  Future<void> _fetchRelatedVisitors() async {
    ...
    try {
      final response = await _client.getRelatedVisitors(filter);
      setState(() {
        _result =
            'Visitors:\n${(response['visitors'] as List).map((v) => v.toString()).join('\n')}\n\nMeta:\n${response['meta'].toString()}';
      });
    } catch (e) {
      setState(() {
        _result = 'Error fetching related visitors: $e';
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('FingerprintJS Related Visitors Test')),
      body: Padding(
